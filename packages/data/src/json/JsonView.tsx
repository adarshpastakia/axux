...
const JsonValue = ({
  ...
  const display = useMemo(() => {
    if (isEmpty(value)) return <label>null</label>;
...
const JsonEmptyObject = ({ type = "object" }) => {
  return <label>{type === "object" ? "{empty object}" : "[empty list]"}</label>;
};
    ...
    <div className="ax-json__property">
      ...
      {type === "object" && (
        <AxCollapsable isCollapsed={false}>
          <label className="ax-json__property--label">{props.labeler?.([...keys, prop].join(".")) ?? prop}</label>
