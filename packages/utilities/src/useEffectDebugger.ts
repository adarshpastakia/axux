...
const compareInputs = (oldInputs: AnyObject[], newInputs: AnyObject[], prefix: string) => {
  // Edge-case: different array lengths
  if (oldInputs.length !== newInputs.length) {
    // Not helpful to compare item by item, so just output the whole array
    console.info(`%c${prefix}::%cInputs have a different length`, TypeColors.info, TagColors.info);
...
export const useEffectDebugger = (func: AnyObject, inputs: AnyObject[], prefix = "useEffect") => {
